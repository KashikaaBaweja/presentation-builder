...
function mapKnownAuthMessage(message: string): string {
  if (message.includes("Error sending confirmation email")) {
    return "Account created, but Supabase could not send the confirmation email. In Supabase → Authentication → Providers → Email, disable “Confirm email” or configure SMTP.";
  }
